...
from bokeh.models import ColumnDataSource, Slider, HoverTool, LinearColorMapper, WMTSTileSource
...
plot.add_tile(
    WMTSTileSource(url="https://cartodb-basemaps-a.global.ssl.fastly.net/light_all/{Z}/{X}/{Y}.png")
)
...
def update_data(month):
    ...
    crime_counts_by_hex = monthly_data.groupby("h3_index").size().reset_index(name="count")
    ...
    color_mapper = LinearColorMapper(palette=extracted_colors, low=crime_counts_by_hex["count"].min(), high=crime_counts_by_hex["count"].max())

    hexagons_source.data = {
        "xs": [list(polygon.exterior.coords.xy[0]) for polygon in hexagons_gdf.geometry],
        "ys": [list(polygon.exterior.coords.xy[1]) for polygon in hexagons_gdf.geometry],
...
# Define a function to automatically update the slider
def auto_update():
...
# Arrange the slider and plot in a column layout
...
curdoc().add_periodic_callback(auto_update, 1000)
